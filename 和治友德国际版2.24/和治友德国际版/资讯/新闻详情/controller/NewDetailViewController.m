...
- (void)viewDidLoad {
    ...
    [self addObserver:self forKeyPath:@"SelectRow" options:NSKeyValueObservingOptionInitial context:nil];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    if ([keyPath isEqualToString:@"SelectRow"]) {
        cellHeight = [self.firstContentArray[self.SelectRow] boundingRectWithSize:CGSizeMake(mainScreenWidth, 2000) options:NSStringDrawingUsesLineFragmentOrigin attributes:[NSDictionary dictionaryWithObject:[UIFont systemFontOfSize:14] forKey:NSFontAttributeName] context:nil].size.height;
...
//            c++;
            if (sender.selected ==YES) {
                ...
                [MBProgressHUD showSuccess:@"已标记" toView:nil];
            ...
            [MBProgressHUD showSuccess:@"标记喜欢" toView:nil];
            sender.selected = YES;
